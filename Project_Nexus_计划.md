...
>
> **当前**: Nexus_brain v0.8.0-dev (向量记忆 ✅) → v0.9.0 计划中 (画像 + 主动工具 🆕)
>
> **总体进度**: █████████████████░░░ 92%
...
| 14 | [v0.9.0 画像 + 主动工具（计划）](#14-v090-画像--主动工具计划) |
...
## 14. v0.9.0 画像 + 主动工具（计划）

> 🆕 计划于 2026-06-24 实施。参考 Iris Chat Memory 的用户画像和 LLM 工具机制。

### 14.1 用户画像自动化（P0-3）

**问题**：当前主人必须手动维护小本本中的「👤 主人信息」段落，容易过时、不完整。

**方案**：从对话中自动提取用户画像，字段独立置信度追踪。

```text
画像字段设计（参考 Iris UserProfile）:

  personality_tags:       ["认真", "幽默", "有点社恐"]
  interests:              ["舞萌", "MAD制作", "AE插件开发"]
  occupation:             学生 / 考研中
  language_style:         简洁直接 / 喜欢用表情包
  communication_preference: 简洁 / 详细 / 随意
  emotional_baseline:     稳定 / 敏感 / 乐观
  important_dates:        [{"date": "12-25", "desc": "考研初试"}]
  taboo_topics:           ["不想聊成绩"]
  bot_relationship:       对 AI 的称呼/关系设定
```
...
| 层级 | 触发条件 | 更新内容 |
|------|---------|---------|
| 短期 | 每次对话后 | 更新简单标签（兴趣提及、情感变化） |
| 中期 | 每 5 次 Tier 2 或 24 小时 | LLM 深度分析最近对话，更新画像字段 |
| 长期 | 每 7 天 | 仅在检测到显著新信息时更新，防止画像漂移 |

**字段独立置信度**：每个字段追踪自己的置信度。高置信度字段不会被低置信度分析结果覆盖。"主人的考研目标是南邮"（置信度 0.95）不会因为一次模糊提及而被改成别的。

**代码量估计**：~400 行新增（`brain/profile.py` + 修改 `brain/memory.py`）

### 14.2 LLM 主动记忆工具（P1-1）

**问题**：LLM 无法在对话中主动存储记忆，完全依赖轮次触发器 `after_turn()` 被动提取。

**方案**：注册 AstrBot LLM Tool，让 LLM 在对话中自己决定什么时候存、存什么。

```python
# 工具 1: save_memory — 对话中主动存记忆
#   触发场景: LLM 判断「主人刚才说的这件事很重要」
#   参数: content, confidence[HIGH/MEDIUM/LOW], category
#   示例: 「主人说下周五是妈妈生日，得提醒他买礼物」

# 工具 2: search_memory — 对话中主动查记忆
#   触发场景: LLM 需要确认「主人之前说过XX吗？」
#   参数: query, top_k
#   示例: 主人问「上次说的那个游戏」→ LLM 调用 search_memory("游戏 舞萌") 

# 工具 3: correct_memory — 修正错误记忆
#   触发场景: 主人纠正了之前的信息
#   参数: memory_id, corrected_content
#   示例: 「不对，我考研目标是北邮不是南邮」→ LLM 修正记忆
```

**代码量估计**：~200 行新增（`brain/tools.py`）

### 14.3 查询改写（P1-3）

**问题**：用户说"上次说的那个游戏"时，「那个游戏」的嵌入向量可能匹配不到「舞萌」。

**方案**：FAISS 检索前用轻量 LLM 调用改写查询。
...
**代码量估计**：~80 行修改（`brain/vector_memory.py`）

### 14.4 实施路线

```
Phase 2 (v0.9.0) — 画像 + 主动工具 (~2-3 天)
  Day 1: P0-3 用户画像自动化 (brain/profile.py)
  Day 2: P1-1 LLM 主动记忆工具 (brain/tools.py)  
  Day 3: P1-3 查询改写 + 联调测试
```
...
| Iris 特性 | Nexus 计划 | 说明 |
|-----------|-----------|------|
| 用户画像 (User Profile) | ✅ P0-3 | 自动提取 + 三级更新 + 字段置信度 |
| 群聊画像 (Group Profile) | ❌ 不适用 | Nexus 单用户场景 |
| LLM Memory Tools | ✅ P1-1 | save/search/correct 三个工具 |
| 查询改写 (Query Rewrite) | ✅ P1-3 | 轻量 LLM 改写提升召回 |
